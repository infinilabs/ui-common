...
import { CaretRightOutlined, LeftOutlined, LoadingOutlined, PauseOutlined, ReloadOutlined, RightOutlined } from "@ant-design/icons";

import dateMath from "./utils/date_math";
...
const DatePicker = (props) => {
  ...
  const getBounds = () => {
    ...
    return {
      ...
      max: endMoment && endMoment.isValid() ? endMoment : dayjs(),
    };
  };
  ...
  const stepForward = () => {
    const { min, max } = getBounds();
    const diff = max.diff(min);
    const endMoment = dayjs(max)
      .add(diff + 1, "ms")
      .tz(timeZone);
    if (endMoment.diff(dayjs().tz(timeZone), "s") > 0) {
      return;
    }
    handleRangeChange({
      start: dayjs(max)
  ...
  return (
    ...
    >
      <Space.Compact className={styles.RangeBox} style={{ width: onRefresh ? 'calc(100% - 72px)' : 'calc(100% - 36px)'}}>
        {!isMinimum && (
          <Button
