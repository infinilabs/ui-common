...
const StartAndEndTimes = props => {
  ...
  const [startAsMoment, setStartAsMoment] = useState(() => {
    ...
    return value && value.isValid() ? value.tz(timeZone) : dayjs().tz(timeZone);
  });
          ...
          <div className={styles.recent}>
            ...
            {recentlyUsedRanges.map((item, index) => (
              <div
                ...
                onClick={() => {
                  onRangeChange({ 
                    start: dayjs(item.start).tz(item.timeZone).tz(timeZone).toISOString(true), 
                    end: dayjs(item.end).tz(item.timeZone).tz(timeZone).toISOString(true)
                  })
                  onCancel()
                }}
              >
