...
export default function isRelativeToNow(timeFrom, timeTo) {
  ...
  const isRelative =
    fromDateMode === DATE_MODES.RELATIVE && toDateMode === DATE_MODES.RELATIVE;
...
export function prettyDuration(
  ...
  if (isRelativeToNow(timeFrom, timeTo)) {
    ...
    let countTimeUnitFullName =
      locales[`datepicker.time.units.${countTimeUnit}`];
    if (
      relativeParts.count === 1 &&
      countTimeUnitFullName.substring(countTimeUnitFullName.length - 1) === "s"
    ) {
      countTimeUnitFullName = countTimeUnitFullName.substring(
        0,
        countTimeUnitFullName.length - 1
      );
    }
