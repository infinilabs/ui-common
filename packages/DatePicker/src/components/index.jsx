...
const DatePicker = (props) => {
  ...
  const stepBackward = () => {
    ...
    handleRangeChange({
      start: moment(min)
        ...
        .tz(timeZone)
        .toISOString(),
      end: moment(min).subtract(1, "ms").tz(timeZone).toISOString(),
    });
  };
  ...
  const stepForward = () => {
    ...
    handleRangeChange({
      start: moment(max).add(1, "ms").tz(timeZone).toISOString(),
  ...
  const isMinimum = useMemo(() => {
    return showTimeSetting && showTimeField && shouldTimeField
      ? !timeField
      : false;
  }, [showTimeSetting, showTimeField, shouldTimeField, timeField]);
  ...
  const [time] = useState(() => fromMilliseconds(refreshInterval));
    ...
    >
      ...
      <Button.Group className={styles.refreshBtn}>
        <Button className={styles.play} onClick={onPlayClick}>
          {isRefreshPaused ? <CaretRightOutlined /> : <PauseOutlined />}
        </Button>
        {onRefresh ? (
          <Button className={styles.play} onClick={onRefreshClick}>
            {reloadLoading ? <LoadingOutlined /> : <ReloadOutlined />}
          </Button>
        ) : null}
      </Button.Group>
    </div>
